package dev.latvian.apps.units.test;

import dev.latvian.apps.units.Unit;
import dev.latvian.apps.units.UnitContext;
import dev.latvian.apps.units.VariableSet;
...
public class MiscTests {
	...
	@Test
	@DisplayName("Functions V.1: sin/time/var combo with MoLang names")
	public void functions51() {
		assertEval("math.sin(roundedTime() * 1.1) * (($test - 32) / 2)", Math.sin(RoundedTimeUnit.time() * 1.1D) * (($test - 32D) / 2D));
	}
